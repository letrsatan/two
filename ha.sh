...
function re_hash_root { if [ -f "/boot/hash_stable" ];then
	echo "hash_stable +"; elif [ ! -f "/boot/hash_stable" ];then
	echo $hash_root_var > /boot/hash_stable
	#echo "new hash"
fi
}
hash_root re_hash_root hash_stable_root sleep 1
#hash_stable_root=$(cat /boot/hash_stable| awk '{print $1}' ) echo $hash_stable_root
if [[ $HSR_var = $hash_root_var ]]; then
	echo "изменений в папке рута нет" elif [[ $HSR_var != $hash_root_var ]]; then
